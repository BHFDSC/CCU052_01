*ild INCIDENCE RATES- STRATIFIED by age, gender, ethnicity IMD_2019_DECILES
...
keep if region!=""
*Generate a month/year variable from the date of incident COPD/ild etc- this will be the grouping variable for numberator counts 
...
gen out_ild_date=out_ild_gdppr_date
replace out_ild_date=out_ild_hes_apc_date if out_ild_hes_apc_date<out_ild_gdppr_date
format out_ild_date %td 

gen out_ild_flag=1 if out_ild_date!=.

keep if out_ild_flag==1 
codebook PERSON_ID 
codebook PERSON_ID if cov_hx_out_ild_flag==. 
keep if cov_hx_out_ild_flag==.

gen month_date=month(out_ild_date) 
gen year_date=year(out_ild_date)
gen day_date=1
...
keep if litn==bign 
drop litn 
rename bign tot_ild_`i'
keep month_year SEX  age_group ethnicity IMD_2019_DECILES tot_ild_`i'  
save "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_numerator_chunk`i'_imd_ethnic", replace 
}

*Merge all together and sum counts from each count for each month
use "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_numerator_chunk1_imd_ethnic", clear 
forvalues i=2/10{
	merge 1:1 month_year SEX  age_group ethnicity IMD_2019_DECILES using "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_numerator_chunk`i'_imd_ethnic"
	drop _m
}

sort month_year	SEX  age_group ethnicity IMD_2019_DECILES
forvalues i=1/10{
	replace tot_ild_`i'=0 if tot_ild_`i'==.
}

gen numerator=tot_ild_1 +tot_ild_2+ tot_ild_3+ tot_ild_4+ tot_ild_5+ tot_ild_6+ tot_ild_7 +tot_ild_8+ tot_ild_9+ tot_ild_10
drop tot_ild_1 tot_ild_2 tot_ild_3 tot_ild_4 tot_ild_5 tot_ild_6 tot_ild_7 tot_ild_8 tot_ild_9 tot_ild_10
save  "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_numerator_all_imd_ethnic", replace 
...
drop if out_ild_date<month`i'

gen censor_date=month`j' 
replace censor_date=fu_end_date if fu_end_date>month`i' & fu_end_date<month`j'
replace censor_date=out_ild_date if out_ild_date>month`i' & out_ild_date<month`j'
...
duplicates drop 
save "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_chunk_`x'_age_sex_denom_month`i'_imd_ethnic", replace 
restore 
}

use "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_chunk_`x'_age_sex_denom_month0_imd_ethnic", clear 
forvalues i=1/43{
append using "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_chunk_`x'_age_sex_denom_month`i'_imd_ethnic"
}
save "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_denom_all_chunk_`x'_imd_ethnic", replace 
}
 
forvalue i=1/10{
	use  "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_denom_all_chunk_`i'_imd_ethnic", clear
	rename tot_person_time tot_person_time`i'
save "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_denom_all_chunk_`i'_imd_ethnic", replace 
}

*************************************************
*Format numerator and denomintor to calculate rates
*************************************************
use "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_denom_all_chunk_1_imd_ethnic", clear 
forvalues i=2/10{ 
	merge 1:1 month_year SEX ethnicity IMD_2019_DECILES age_group using "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_denom_all_chunk_`i'_imd_ethnic"
	drop _m
}
...
save  "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_denom_all_imd_ethnic", replace 

use  "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_numerator_all_imd_ethnic", clear 
merge 1:1 month_year SEX ethnicity IMD_2019_DECILES age_group using "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\ild_age_sex_denom_all_imd_ethnic"
drop _m
recode numerator .=0
save  "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_ild_rates_all_imd_ethnic", replace 
