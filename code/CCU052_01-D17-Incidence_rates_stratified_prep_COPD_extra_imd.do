*COPD INCIDENCE RATES- STRATIFIED by age, gender, region
...
gen out_copd_date=out_copd_gdppr_date
replace out_copd_date=out_copd_hes_apc_date if out_copd_hes_apc_date<out_copd_gdppr_date
format out_copd_date %td 

gen out_copd_flag=1 if out_copd_date!=.

keep if out_copd_flag==1 
codebook PERSON_ID 
codebook PERSON_ID if cov_hx_out_copd_flag==. 
keep if cov_hx_out_copd_flag==.

gen month_date=month(out_copd_date) 
gen year_date=year(out_copd_date)
gen day_date=1
gen month_year=mdy(month_date, day_date, year_date)
format month_year %td

*generating age. Stratified by baseline age? With the standardised rates does age need to be updated every year/month? Also, what is everyone's age cut off for COPD-40? use 40 copd, 40 for ild
sort month_year
...
keep if litn==bign 
drop litn 
rename bign tot_copd_`i'
keep month_year SEX  age_group ethnicity IMD_2019_DECILES tot_copd_`i'  
save "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_numerator_chunk`i'_imd_ethnic", replace 
}

*Merge all together and sum counts from each count for each month
use "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_numerator_chunk1_imd_ethnic", clear 
forvalues i=2/10{
	merge 1:1 month_year SEX  age_group ethnicity IMD_2019_DECILES using "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_numerator_chunk`i'_imd_ethnic"
	drop _m
}

sort  month_year SEX  age_group ethnicity IMD_2019_DECILES
forvalues i=1/10{
	replace tot_copd_`i'=0 if tot_copd_`i'==.
}

gen numerator=tot_copd_1 +tot_copd_2+ tot_copd_3+ tot_copd_4+ tot_copd_5+ tot_copd_6+ tot_copd_7 +tot_copd_8+ tot_copd_9+ tot_copd_10
drop tot_copd_1 tot_copd_2 tot_copd_3 tot_copd_4 tot_copd_5 tot_copd_6 tot_copd_7 tot_copd_8 tot_copd_9 tot_copd_10
save  "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_numerator_all_imd_ethnic", replace 
...
drop if out_copd_date<month`i'

gen censor_date=month`j' 
replace censor_date=fu_end_date if fu_end_date>month`i' & fu_end_date<month`j'
replace censor_date=out_copd_date if out_copd_date>month`i' & out_copd_date<month`j'
format censor_date %td

gen person_time=censor_date-month`i'
keep PERSON_ID month`i' SEX   ethnicity IMD_2019_DECILES DOB person_time
...
by SEX age_group ethnicity IMD_2019_DECILES: gen tot_person_time`i'=sum(person_time)
by SEX age_group ethnicity IMD_2019_DECILES: gen litn=_n
by SEX age_group ethnicity IMD_2019_DECILES: gen bign=_N 
keep if litn==bign 
keep month`i' tot_person_time`i' SEX age_group ethnicity IMD_2019_DECILES
rename month`i' month_year
duplicates drop 
save "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\chunk_`x'_age_sex_denom_month`i'_imd_ethnic", replace 
restore 
}


use "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\chunk_`x'_age_sex_denom_month0_imd_ethnic", clear 
forvalues i=1/43{
append using "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\chunk_`x'_age_sex_denom_month`i'_imd_ethnic"
}
save "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_denom_all_chunk_`x'_imd_ethnic", replace 
}
...
*************************************************
forvalues x=1/10{
use "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_denom_all_chunk_`x'_imd_ethnic", clear 
forvalues i=1/43{
	replace tot_person_time0=tot_person_time`i' if tot_person_time`i'!=. & tot_person_time0==.
}
drop tot_person_time1-tot_person_time43
rename tot_person_time0 tot_person_time`x'
save  "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_denom_all_chunk_`x'_imd_ethnic", replace 
} 

use "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_denom_all_chunk_1_imd_ethnic", clear 
forvalues i=2/10{ 
	merge 1:1 month_year SEX  age_group ethnicity IMD_2019_DECILES using "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_denom_all_chunk_`i'_imd_ethnic"
	drop _m
}
...
save  "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_denom_all_imd_ethnic", replace 

use  "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_numerator_all_imd_ethnic", clear 
merge 1:1 month_year SEX  age_group ethnicity IMD_2019_DECILES using "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_denom_all_imd_ethnic"
drop _m
recode numerator .=0
save  "D:\PhotonUser\My Files\Home Folder\StataFiles\CCU052\age_sex_COPD_rates_all_imd_ethnic", replace 
